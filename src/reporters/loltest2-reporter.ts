...
const pad = (str: string, len: number): string =>
    str.trim().padEnd(len).padStart(len + (str.length / 2));
...
const logSuccess = (title: string, fileName: string, duration: number) =>
    `${badge(
        BgColor.Green,
        "PASS"
    )} ${colorize(Effect.Bold, fileName)} ${dim("›")} ${title}${time(duration)}`;
...
const LolTest2Reporter: LolTest2Reporter = {
    ...
    onRunStart: ({ total, numFiles }) =>
        `${colorize(Effect.Underline,
            `Running ${total} ${pluralize("test", total)} in ${numFiles} ${pluralize(
            "file",
            numFiles
        )}...\n`)}`,
    ...
    onRunComplete({ total, passed, failed, duration, numFiles }): string {
        ...
        const fails = this.fails.map(({ testCase, error, duration }, idx) => [
            testCase
                ? `${INDENT}${red(`${idx + 1}) ${testCase.fileName}`)} ${dim("›")} ${
                    red(testCase.title)}${duration ? ` (${formatTime(duration)})` : ''}\n`
                : undefined,
        ...
        return [
            ...
            `${colorize(Effect.Bold, 'Files')}:\t\t${[
                ...
                this.passedFiles.size ? green(`${
                    this.passedFiles.size} passed`) : `${this.passedFiles.size} passed`,
            ...
            `${colorize(Effect.Bold, 'Duration')}:\t${formatTime(duration)} (${
